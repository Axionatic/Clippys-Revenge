...
- `bin/clippy` — Shell wrapper that sets `PYTHONPATH`, reads `$INSTALL_DIR/.python-bin` (absolute interpreter path written by `install.sh`), and execs that interpreter with `-m clippy.launcher`. Falls back to `python3` with a stderr warning if the manifest is missing or its interpreter has been removed.
- `install.sh` / `uninstall.sh` — Install to `~/.local/share/clippys-revenge`, symlink `bin/clippy` to `~/.local/bin/clippy`. Uses `tools/find_python.sh` to probe for Python ≥3.10 (env override → version-pinned `python3.14/.13/.12/.11/.10` → brew paths → uv-managed CPython glob → `python3` last). On macOS without a usable Python the installer offers an opt-in uv bootstrap (downloads `astral.sh/uv/install.sh`, then `uv python install 3.13`). Marker files under `INSTALL_DIR` (`.uv-installed-by-us`, `.uv-python-installed-by-us`, `.tattoy-installed-by-us`, `.installed-via-tarball`) tell `uninstall.sh` which third-party tools we own and may safely offer to remove. **Hard rule:** uninstall never touches user-owned tooling (cargo, Homebrew, Xcode CLT, system Python).
- `tools/find_python.sh` — Pure-bash Python discovery used by `install.sh` and exercised standalone by `tests/test_find_python.sh`. Sourced as a function (`find_python` → sets `PYTHON_BIN`) or run directly (prints path, exit 0/1).

**Python interpreter resolution:** `install.sh` writes the absolute interpreter path to `$INSTALL_DIR/.python-bin`. `bin/clippy` reads that file and execs the recorded interpreter. Plugin scripts (effects + `unified_runner.py`) get an absolute shebang derived from `sys.executable` (see `ensure_executable` in `clippy/launcher.py`), so tattoy spawns plugins under the same interpreter as the launcher regardless of tattoy's own `PATH`. The result: a uv-managed Python isolated under `~/.local/share/uv/python/...` works end-to-end without touching system `PATH` ordering.
